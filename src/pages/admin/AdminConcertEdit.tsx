...
const dotToIso = (dot: string | null) => {
  if (!dot) return "";
  return dot.replace(/\./g, "-");
};

export default function AdminConcertEdit() {
  ...
  useEffect(() => {
    ...
    const fetchConcert = async () => {
      ...
      setVenue(data.venue ?? "");
      setArea(data.area ?? "");
      setStartDate(dotToIso(data.start_date));
      setEndDate(dotToIso(data.end_date));
      setStatus(data.status ?? "공연예정");
      ...
      if (Array.isArray(data.intro_images)) {
        setIntroImages(data.intro_images as string[]);
      }
      if (Array.isArray(data.ticket_sites)) {
        setTicketSites(
          (data.ticket_sites as { name: string; url: string }[]).map((s) => ({
            name: s.name ?? "",
            url: s.url ?? "",
          }))
        );
  ...
  const handleIntroUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const ext = file.name.split(".").pop();
      const path = `${id}/intro-${Date.now()}-${i}.${ext}`;
      const { error: uploadError } = await supabase.storage
        .from("concerts")
        .upload(path, file);
      if (uploadError) {
      ...
      .update({
        ...
        intro_images:
          introImages.filter((u) => u.trim()).length > 0
            ...
            : null,
        ticket_sites:
          ticketSites.filter((s) => s.name.trim() && s.url.trim()).length > 0
  ...
  if (initialLoading) {
    return (
      <div className="admin-concert-new">
        <div className="wrap">
          <p style={{ padding: "40px 0", color: "#888" }}>불러오는 중...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="admin-concert-new">
      <div className="wrap">
        <h1 className="admin-concert-new__title">공연 정보 수정</h1>
        ...
        <div className="admin-concert-new__form">
          ...
          <div className="admin-concert-new__actions">
            <Button
              variant="outline"
              onClick={() => navigate(`/concert-info/${id}`)}
            >
              취소
            </Button>
            <Button
